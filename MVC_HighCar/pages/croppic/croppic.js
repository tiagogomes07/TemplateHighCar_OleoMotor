...
(function (window, document) {
	...
	Croppic.prototype = {
		...
		bindImgUploadControl: function(){
			...
			if( !$.isEmptyObject(that.croppedImg)){
				...
				that.cropControlRemoveCroppedImage.on('click',function(){ 
					...
					//var urlImagem = that.imgUrl;

					//var urlImagem = $("#yourId img").attr("src").split("/")[2];
					//$("#yourId img").attr("src") = "";
	
					// $.ajax(
					// { url: '../croppic/script.php?filename='+urlImagem }
					// );
		...
		reset:function(){
			...
			if(that.id = "photo_Materia"){
				LoadImagemMateria(null);
				$("#photo_Materia img").remove()
			}
